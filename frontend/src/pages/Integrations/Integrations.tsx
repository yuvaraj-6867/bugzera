  ...
  const connect = async () => {
    ...
    if (data.url) {
      ...
      const timer = setInterval(() => {
        if (popup?.closed) { clearInterval(timer); fetchStatus(); setLoading(false) }
      }, 500)
    } else { setLoading(false) }
  }

  const disconnect = async (id: number) => {
    if (!await confirmDialog('Disconnect this Google account?', 'Disconnect')) return
    await fetch(`/api/v1/google_calendar/disconnect?account_id=${id}`, { method: 'DELETE', headers })
    fetchStatus()
  }
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start mb-1">
            <h3 className="text-xl font-bold text-[#0F172A]">Google Calendar</h3>
            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${accounts.length > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
              {accounts.length > 0 ? 'active' : 'inactive'}
            </span>
          </div>
          <p className="text-sm text-gray-600 mb-2">Sync Google Calendar events and manage multiple accounts.</p>
          {accounts.length > 0 && (
            <div className="mb-3 space-y-1">
              {accounts.map(acc => (
                <div key={acc.id} className="flex items-center justify-between text-xs bg-gray-50 rounded px-2 py-1">
                  ...
                  <button onClick={() => disconnect(acc.id)} className="text-red-400 hover:text-red-600 ml-2">Disconnect</button>
                </div>
              ))}
            </div>
          )}
          <button onClick={connect} disabled={loading} className="btn btn-primary text-sm">
            {loading ? 'Connecting…' : accounts.length > 0 ? '+ Add Account' : 'Connect'}
          </button>
        </div>
      </div>
    </div>
