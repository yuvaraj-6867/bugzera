...
const Calendar = () => {
  ...
  const gcalEmail     = gcalAccounts[0]?.email || ''
  ...
  const disconnectGcal = async (accountId?: number) => {
    if (!await confirmDialog('Disconnect Google Calendar?', 'Disconnect')) return
    try {
      const url = accountId ? `/api/v1/google_calendar/disconnect?account_id=${accountId}` : '/api/v1/google_calendar/disconnect'
    ...
    <div className="flex h-[calc(100vh-57px)] overflow-hidden bg-white text-gray-900">
      ...
      <div className="w-56 shrink-0 p-4 border-r border-gray-200 flex flex-col gap-4 overflow-y-auto bg-gray-50">
        ...
        <div className="mt-auto border-t border-gray-200 pt-3">
          ...
          {gcalAccounts.map(acc => (
            <div key={acc.id} className="flex items-center gap-1.5 px-1 mb-1.5 group">
              ...
              <span className="text-xs text-gray-600 truncate flex-1">{acc.email}</span>
              <button onClick={() => disconnectGcal(acc.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition text-xs">✕</button>
            </div>
          ))}
          <button
            onClick={connectGcal}
            disabled={gcalLoading}
            className="w-full flex items-center justify-center gap-2 py-2 rounded-lg border border-gray-300 text-xs font-semibold text-gray-600 hover:bg-gray-100 disabled:opacity-60 transition mt-1"
          >
            ...
            {gcalLoading ? 'Connecting…' : gcalConnected ? 'Add Account' : 'Connect Google Calendar'}
          </button>
        </div>
